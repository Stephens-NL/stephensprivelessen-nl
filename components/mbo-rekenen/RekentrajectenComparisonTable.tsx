              ...
              <div className="space-y-3">
                {rekentrajectenComparison.features.map((feature) => (
                  <div key={feature.id} className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-[var(--ink)]">{getCourseValue(course, feature.id, language)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <tr className="border-b border-[var(--border-warm)]">
              ...
              {rekentrajectenComparison.courses.map((course) => (
                ...
                >
                  ...
                  <div className="text-3xl font-black text-[var(--ink)]">{course.pricePerPerson}</div>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rekentrajectenComparison.features.map((feature, index) => (
              <m.tr
                ...
                className="border-b border-[var(--border-warm)] hover:bg-[var(--cream)]/50 transition-colors duration-200"
              >
                <td className="p-6 text-[var(--ink)] font-bold">{feature.label[language]}</td>
                {rekentrajectenComparison.courses.map((course) => (
                  <td
                  ...
                  >
                    <span
                      className={
                        feature.id === 'pricePerPerson' ? 'text-2xl font-black text-[var(--ink)]' : 'text-[var(--warm-text)]'
                      }
                    >
