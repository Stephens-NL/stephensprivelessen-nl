...
const QuestionComponent: React.FC<{ question: Question; onChange: (id: string, value: any) => void; value: any; onNext: () => void; formData: Record<string, any> }> = ({ question, onChange, value, onNext, formData }) => {
  ...
  // Check if the question should be shown based on its conditional property
...
export const FeedbackSystem: React.FC<{ longVersion: FeedbackForm; shortVersion: FeedbackForm }> = ({ longVersion, shortVersion }) => {
  const [currentStep, setCurrentStep] = useState(-2); // Start at -3 for language selection
  ...
  const selectFormLength = (isLong: boolean) => {
    setSelectedForm(isLong ? longVersion : shortVersion);
    setCurrentStep(0);
  };
          ...
          {isSubmitted ? (
            ...
            >
              <h1 className="text-3xl font-bold text-white mb-6">
                <FadeInText text={t(selectedForm ? selectedForm.title : welcomeScreenData.lengthSelection.title)} />
              </h1>
              {renderContent()}
