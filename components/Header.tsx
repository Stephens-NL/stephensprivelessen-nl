...
function navLinkClass(pathname: string, href: string, variant: 'desktop' | 'mobile') {
    const isActive = pathname === href;
    const base = variant === 'desktop'
        ? 'px-3 py-1.5 text-sm font-medium transition-all duration-200 rounded-md whitespace-nowrap'
        : 'block px-3 py-2.5 text-sm font-medium rounded-md transition-colors';
    return `${base} ${isActive ? 'text-amber bg-white/5' : 'text-cream/70 hover:text-cream hover:bg-white/5'}`;
}
                ...
                >
                    <m.div
                        ...
                        animate={{
                            ...
                            /* rgba(13, 40, 24, 0.97) = --ink at 97% opacity */
