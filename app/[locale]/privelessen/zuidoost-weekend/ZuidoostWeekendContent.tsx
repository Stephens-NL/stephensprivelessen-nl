        ...
        <div className="container mx-auto px-4 py-16">
          <div className="text-center text-white">
            <div className="inline-block bg-[var(--amber-hover)]/20 rounded-full px-6 py-2 mb-6">
              <div className="flex items-center gap-2 text-[var(--amber)] font-bold"><FaStar /><span>{t(content.specialOffer)}</span><FaStar /></div>
            </div>
            ...
            <div className="mb-12">
              <h2 className="text-4xl md:text-5xl font-bold text-[var(--amber)] mb-2">{t(content.discount.text)}</h2>
              <p className="text-xl text-[var(--cream)]">{t(content.discount.subtext)}</p>
            </div>
            <div className="flex flex-col md:flex-row gap-6 justify-center items-center mb-12">
              <div className="bg-white/10 rounded-xl p-6 backdrop-blur-sm w-full md:w-64">
                <h3 className="text-lg font-medium text-[var(--cream)] mb-2">{t(content.pricing.regularPrice.label)}</h3>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <m.div className="bg-gradient-to-br from-[var(--amber)] to-[var(--amber-hover)] rounded-xl p-6 w-full md:w-72 transform hover:scale-105 transition-transform duration-300 cursor-pointer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    ...
                    <div className="bg-[var(--amber)] text-[var(--ink)] text-sm font-bold py-1 px-3 rounded-full inline-block">{t(content.pricing.communityRate.savings)}</div>
                  </m.div>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[425px] bg-amber-900 text-white">
                  <DialogHeader><DialogTitle className="text-2xl font-bold text-[var(--amber)] mb-4">Student Information</DialogTitle></DialogHeader>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-12">
              <div className="bg-white/10 rounded-xl p-6 backdrop-blur-sm">
                <div className="flex items-center gap-2 text-[var(--amber)] mb-3"><FaMapMarkerAlt className="text-xl" /><h3 className="font-medium">{t(content.features.location.title)}</h3></div>
                <p className="text-white/90">{t(content.features.location.text)}</p>
              </div>
              <div className="bg-white/10 rounded-xl p-6 backdrop-blur-sm">
                <div className="flex items-center gap-2 text-[var(--amber)] mb-3"><FaClock className="text-xl" /><h3 className="font-medium">{t(content.features.availability.title)}</h3></div>
                <p className="text-white/90">{t(content.features.availability.text)}</p>
              </div>
              <div className="bg-white/10 rounded-xl p-6 backdrop-blur-sm">
                <div className="flex items-center gap-2 text-[var(--amber)] mb-3"><FaCheck className="text-xl" /><h3 className="font-medium">{t(content.features.extras.title)}</h3></div>
            ...
            <m.p className="text-3xl font-bold text-[var(--amber)]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>{t(content.footer)}</m.p>
            <m.div className="mt-16 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
              ...
              <Dialog>
                <DialogTrigger asChild><Button size="lg" className="bg-[var(--amber-hover)] hover:bg-[var(--amber)] text-[var(--ink)] font-semibold px-8 py-6 text-lg">Contact Us Now</Button></DialogTrigger>
                <DialogContent className="sm:max-w-[425px] bg-amber-900 text-white">
                  <DialogHeader><DialogTitle className="text-2xl font-bold text-[var(--amber)] mb-4">Student Information</DialogTitle></DialogHeader>
